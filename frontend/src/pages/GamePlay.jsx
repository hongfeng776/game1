...
const ITEM_DISPLAY = {
  hp_potion_small: { name: '小型生命药水', icon: '🧪' },
  hp_potion_medium: { name: '中型生命药水', icon: '🧴' },
  hp_potion_large: { name: '大型生命药水', icon: '⚗️' },
  attack_boost: { name: '狂暴药剂', icon: '💪' },
  defense_boost: { name: '铁壁药剂', icon: '🛡️' },
  revive: { name: '复活卷轴', icon: '📜' },
  speed_boost: { name: '疾风药剂', icon: '💨' },
  shield: { name: '护盾水晶', icon: '💎' }
};
...
function GamePlay() {
  ...
  const handleRevive = async () => {
    try {
      const reviveItem = inventory?.find(i => i.itemId === 'revive' && i.quantity > 0);
      if (reviveItem) {
        await inventoryAPI.useItem('revive', 1);
        await loadInventory();
  ...
  const hasReviveItem = inventory?.some(i => i.itemId === 'revive' && i.quantity > 0) && !game.wasRevived;
  ...
  const usableItems = inventory?.filter(inv => {
    const categories = ['heal', 'buff', 'shield'];
    return categories.includes(inv.item?.category) && inv.quantity > 0;
  }) || [];

  const reviveItem = inventory?.find(inv => inv.itemId === 'revive' && inv.quantity > 0);
      ...
      <div className="game-container">
        <div className="game-sidebar">
          <div className="player-stats-card card">
            ...
            {game.shield > 0 && (
              <div className="stat-row">
                ...
                <span className="stat-value shield">{game.shield}</span>
              </div>
            )}
            <div className="stat-row">
              <span className="stat-icon">⚔️</span>
            ...
            <div className="stat-row">
              ...
              <span className="stat-value">
                {game.defense}
                {game.buffs.defenseBoost > 0 && (
                  <span className="buff-positive"> (+{Math.round(game.buffs.defenseBoost * 100)}%)</span>
                )}
              </span>
            </div>
            {hasReviveItem && (
          ...
          <div className="quick-items-card card">
            <h3>快捷道具</h3>
            {usableItems.length === 0 && !reviveItem ? (
              <p className="no-items">暂无可用道具</p>
            ) : (
              <div className="quick-items-grid">
                {reviveItem && (
                  <button 
                    className={`quick-item ${game.wasRevived ? 'used' : ''}`}
                    title={reviveItem.item.name}
                    disabled={game.wasRevived}
                  >
                    <span className="item-icon">{reviveItem.item.icon}</span>
                    <span className="item-count">{reviveItem.quantity}</span>
                  </button>
                )}
                {usableItems.map(inv => (
                  <button 
                    ...
                    onClick={() => handleUseItem(inv.itemId, inv.item)}
                  >
          ...
          <div className="level-rewards-card card">
            ...
            <div className="reward-item">
              <span className="reward-icon">🎁</span>
              <span className="reward-text">随机道具掉落</span>
            </div>
          </div>
          ...
          <div className="mobile-controls">
            <div className="mobile-control-row">
              <button 
                ...
                onClick={() => game.movePlayer(0, -1, handleRevive, hasReviveItem)}
            ...
            <div className="mobile-control-row">
              <button 
                className="mobile-control-btn"
                onClick={() => game.movePlayer(-1, 0, handleRevive, hasReviveItem)}
                disabled={!game.isPlaying || game.isPaused || game.isGameOver}
              >
                ←
              </button>
          ...
          <div className="inventory-modal card" onClick={e => e.stopPropagation()}>
            ...
            {inventory.length === 0 ? (
              ...
              <div className="inventory-modal-grid">
                {inventory.map(inv => {
                  const usable = ['heal', 'buff', 'shield'].includes(inv.item?.category);
                  return (
                    <div 
                      key={inv.itemId}
                      className={`inventory-modal-item ${!usable ? 'disabled' : ''}`}
                      onClick={() => usable && handleUseItem(inv.itemId, inv.item)}
                    >
                      ...
                      {inv.item?.rarity && (
                        <div className={`item-rarity ${inv.item.rarity}`}>
                          ...
                          {inv.item.rarity === 'epic' && '史诗'}
                          {inv.item.rarity === 'legendary' && '传说'}
                        </div>
                      )}
                    </div>
                  );
